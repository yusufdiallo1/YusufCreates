...
export function AvailabilityBadge({ className }: { className?: string }) {
  ...
  return (
    <Link
      ...
      className={
        className ??
        "inline-flex items-center gap-2 rounded-full border border-[color:var(--border-hairline)] px-3 py-1.5 text-xs text-secondary transition-colors duration-fast hover:text-primary"
      }
    >
      <span
        aria-hidden="true"
