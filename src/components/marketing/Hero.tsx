...
export function Hero({ projects = [] }: { projects?: HeroProject[] }) {
  ...
  // Hold the sequence until the session check resolves, so a returning visitor
  // never sees a flash of the animated-in state.
  if (!checked) return <section ref={ref} className="min-h-[86dvh]" />;
